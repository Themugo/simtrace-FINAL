...
// @ts-ignore - Leaflet type definition issue
...
export default function LiveMap({ token, deviceImeis = [], showAll = false, style: extraStyle = {} }: LiveMapProps) {
    ...
    async function fetchInitial() {
      try {
        ...
        const entries = await Promise.all(
          devs.filter((d: any) => showAll || deviceImeis.includes(d.imei)).map(async (d: any) => {
            try {
              const pr = await fetch(`${BASE}/api/imei/${d.imei}/history?limit=1`, { headers });
              const pings = await pr.json();
              const p = pings[0];
              if (!p) return null;
              return [d.imei, { lat: p.lat, lng: p.lng, ts: p.ts, status: d.status, make: d.make, model: d.model }] as [string, Position];
            } catch { return null; }
          })
        );
        ...
        console.warn("[LiveMap] Failed to fetch initial positions:", err instanceof Error ? err.message : String(err));
      }
    }
    fetchInitial();
  ...
  const markerList: [string, Position][] = Object.entries(positions).filter((entry): entry is [string, Position] => Boolean(entry[1] && entry[1].lat && entry[1].lng));
  const stolenList: [string, Position][] = markerList.filter(([, p]) => p.status === "stolen" || p.status === "blacklisted");
    ...
    <div style={{ position: "relative" }}>
      ...
      >
        ...
        {markerList.map(([imei, pos]) => (
          <Marker key={imei} position={[pos.lat, pos.lng]} icon={ICONS[pos.status as keyof typeof ICONS] || ICONS.active}>
            <Popup>
      ...
      <div style={{ display: "flex", gap: "0.75rem", marginTop: "0.5rem", flexWrap: "wrap" }}>
        {([["🟢","Active"],["🔴","Stolen"],["🟠","Blacklisted"],["🔵","Recovered"]] as const).map(([dot,label]) => (
