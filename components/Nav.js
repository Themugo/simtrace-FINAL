...
export default function Nav() {
  ...
  useEffect(() => {
    function handler(e) { if (adminRef.current && !adminRef.current.contains(e.target)) setAdminMenu(false); }
                ...
                <div style={{
                  ...
                  {adminLinks.map(l => (
                    <Link key={l.href} href={l.href} onClick={() => setAdminMenu(false)}
                      ...
                      onMouseOver={e => { if (!isActive(l.href)) e.currentTarget.style.background = "var(--surface)"; }}
                      onMouseOut={e  => { if (!isActive(l.href)) e.currentTarget.style.background = "transparent"; }}>
                      {l.label}
                    </Link>
                  ))}
                </div>
