...
interface ToastContextType {
  add: (message: string, type?: string, duration?: number) => number;
...
export function ToastProvider({ children }: { children: ReactNode }) {
  ...
  const add = useCallback((message: string, type: string = 'info', duration: number = 5000) => {
    const id = ++toastIdCounter;
    setToasts(t => [...t, { id, message, type: type as Toast['type'], duration }]);
...
export const useToast = () => useContext(ToastCtx) as ToastContextType;
