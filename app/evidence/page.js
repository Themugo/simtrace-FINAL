...
export default function EvidencePage() {
  ...
  async function loadData() {
    try {
      ...
      const stolen = devs.filter(d => d.status === "stolen" || d.status === "blacklisted");
      setDevices(stolen);
      ...
      for (const d of stolen) {
        try {
          ...
          (imgs.evidence || []).forEach(e => allEvidence.push({ ...e, device: d }));
        } catch { /* no evidence yet — normal */ }
      }
      setEvidence(allEvidence.sort((a,b) => new Date(b.capturedAt) - new Date(a.capturedAt)));
    } catch (err) { toast?.add('Failed to load evidence: ' + err.message, 'danger'); }
