    ...
    <div>
      ...
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: "1.5rem", flexWrap: "wrap", gap: "0.75rem" }}>
        <div>
          ...
          <p style={{ color: "var(--emerald)", fontSize: "0.82rem", fontWeight: 600, marginBottom: "0.2rem" }}>
            🎁 Policy: 1 Free Device Slot Per User Account
          </p>
          {limit && (
            <p className="text-muted">
              ...
              {limit.slotsRemaining > 0
                ...
                : <span style={{ color: "var(--amber)", marginLeft: 6 }}>· Free limit reached ($2 fee for extra device)</span>
              }
        ...
        <div style={{ display: "flex", gap: "0.6rem" }}>
          {atLimit && (
            <Link href="/pricing" style={{ background: "var(--amber)22", border: "1px solid var(--amber)55", color: "var(--amber)", borderRadius: 9, padding: "7px 14px", fontSize: "0.82rem", fontWeight: 600, textDecoration: "none" }}>
              ⚡ Buy Slot ($2/mo) / Upgrade
            </Link>
          )}
          <button className={atLimit ? "btn-ghost" : "btn-primary"} onClick={() => setShowForm(s => !s)} disabled={(atLimit ?? false) && !showForm}>
            {showForm ? "Cancel" : atLimit ? "+ Add Additional Device ($2 Fee)" : "+ Register Device (1 Free)"}
          </button>
        </div>
      </div>
      ...
      {/* Policy Banner */}
      {devices.length >= 1 && (
        <div style={{ background: "var(--surface)", border: "1px solid var(--amber)44", borderRadius: 12, padding: "0.85rem 1.1rem", marginBottom: "1.25rem", display: "flex", alignItems: "center", justifyContent: "space-between", flexWrap: "wrap", gap: "0.75rem" }}>
          <div>
            <div style={{ fontWeight: 700, fontSize: "0.88rem", color: "var(--amber)", display: "flex", alignItems: "center", gap: 6 }}>
              <span>ℹ️</span> Additional Device Pricing (System Admin Fee)
            </div>
            <div style={{ fontSize: "0.8rem", color: "var(--text2)", marginTop: 2 }}>
              Your 1st device is 100% free. Registering additional devices incurs a <strong>$2.00 / KES 260 monthly fee</strong> set by System Admin.
            </div>
          </div>
          <Link href="/pricing" className="btn-ghost" style={{ fontSize: "0.78rem", padding: "5px 12px", textDecoration: "none" }}>
            View Fee Details →
          </Link>
        </div>
      )}
      ...
      {/* Forensic SDK Integration Banner */}
      <div className="card" style={{ marginTop: "1.5rem", background: "linear-gradient(135deg, rgba(14, 165, 233, 0.08) 0%, rgba(168, 85, 247, 0.08) 100%)", borderColor: "rgba(56, 189, 248, 0.3)", display: "flex", alignItems: "center", justifyContent: "space-between", flexWrap: "wrap", gap: "1rem" }}>
        <div>
          <div style={{ fontWeight: 700, fontSize: "0.92rem", color: "#f8fafc", display: "flex", alignItems: "center", gap: "0.5rem" }}>
            <span style={{ color: "#38bdf8" }}>⚡</span> Embed SIMTRACE Forensic SDK in Mobile & OEM Firmware
          </div>
          <div style={{ fontSize: "0.82rem", color: "var(--muted)", marginTop: 4 }}>
            Sub-second SIM swap detection, hardware enclave binding, baseband listeners & court-admissible evidence capture.
          </div>
        </div>
        <Link href="/sdk" className="btn-primary" style={{ fontSize: "0.82rem", padding: "0.55rem 1rem", textDecoration: "none" }}>
          View Forensic SDK Guide →
        </Link>
      </div>
