...
import HardwareHealthSection from "../../../components/HardwareHealthSection";
...
export default function DeviceDetailPage() {
  ...
  const tabs = ["overview","hardware","map","pings","reports"];
    ...
    <div style={{ maxWidth:900, margin:"0 auto" }}>
      ...
      {tab === "overview" && (
        <div style={{ display:"flex", flexDirection:"column", gap:"1.25rem" }}>
                  ...
                  <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:"0.4rem" }}>
                    {Object.entries(device.fingerprint).filter(([,v])=>v).map(([k,v]) => (
                      <div key={k} style={{ background:"var(--bg)", borderRadius:7, padding:"0.45rem 0.75rem" }}>
                        ...
                        <div style={{ fontSize:"0.78rem", color:"var(--text2)", fontFamily:"var(--mono)", overflow:"hidden", textOverflow:"ellipsis", whiteSpace:"nowrap" }}>{String(v)}</div>
                      </div>
                    ))}
                  </div>
                </div>
          ...
          <HardwareHealthSection fingerprint={device.fingerprint} make={device.make} model={device.model} />
        </div>
      )}
