...
class SocketClient {
  ...
  onCooperationAlert(callback: (data: { alertId: string; message: string }) => void) {
    this.socket?.on('cooperation:alert', callback);
  }
  ...
  offCooperationAlert(callback: (data: any) => void) {
    this.socket?.off('cooperation:alert', callback);
  }
