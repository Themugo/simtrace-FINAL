...
SimTrace keeps one root variable reference at `.env.example`. For a self-hosted /
Docker production run, copy it and fill in production values:

```bash
cp .env.example .env.production
```

> For the managed deploy (Render + Vercel) you don't create this file at all — set
> the same variables directly in the Render service and Vercel project dashboards.
> The list below is the reference for what to set in either case.
