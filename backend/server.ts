...
import { addCorrelationId } from "./services/correlation.js";
import { setApiVersion } from "./services/apiVersion.js";
...
const trackLimiter = createRateLimiter({ windowMs: 60 * 1000, max: 120, message: { error: "Ping rate limit exceeded" }, useRedis: true });
...
function mpesaIpWhitelist(req: any, res: any, next: any) {
...
// ── Routes ────────────────────────────────────────────────────────────────────
app.get("/health", (_, res) => res.json({ status: "ok", ts: Date.now(), env: isProd ? "production" : "development", version: "v1" }));
