...
router.post('/:id/lock', authenticate, async (req: Request, res: Response, next: NextFunction) => {
  try {
...
router.post('/:id/unlock', authenticate, async (req: Request, res: Response, next: NextFunction) => {
  try {
    const device = await Device.findOne({ _id: req.params.id, owner: req.user!.id });
...
// PATCH /api/devices/:id/commands/:cmdId
