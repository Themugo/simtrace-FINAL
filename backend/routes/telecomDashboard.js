// routes/telecomDashboard.js - Telecom dashboard API endpoints
...
router.get("/:dashboardId", authenticate, async (req, res, next) => {
  try {
    ...
    const dashboard = await getTelecomDashboard(dashboardId);
    res.json(dashboard);
  } catch (err) { next(err); }
});
...
router.get("/company/:companyId", authenticate, async (req, res, next) => {
  try {
    ...
    const dashboard = await getTelecomDashboardByCompany(companyId);
    res.json(dashboard);
  } catch (err) { next(err); }
});
...
router.patch("/:dashboardId", authenticate, async (req, res, next) => {
  try {
    const { dashboardId } = req.params;
    const dashboard = await updateTelecomDashboard(dashboardId, req.body, req.user.id);
    res.json(dashboard);
  } catch (err) { next(err); }
});
...
router.patch("/:dashboardId/widgets", authenticate, async (req, res, next) => {
  try {
    ...
    const { dashboardId } = req.params;
    const data = schema.parse(req.body);
    const dashboard = await updateDashboardWidgets(dashboardId, data.widgets, req.user.id);
    res.json(dashboard);
  } catch (err) {
    if (err.name === "ZodError") return res.status(400).json({ error: err.errors });
    next(err);
...
router.patch("/:dashboardId/settings", authenticate, async (req, res, next) => {
  try {
    ...
    const { dashboardId } = req.params;
    const data = schema.parse(req.body);
    const dashboard = await updateDashboardSettings(dashboardId, data.settings, req.user.id);
    res.json(dashboard);
  } catch (err) {
    if (err.name === "ZodError") return res.status(400).json({ error: err.errors });
    next(err);
...
router.post("/:dashboardId/users", authenticate, async (req, res, next) => {
  try {
    ...
    const { dashboardId } = req.params;
    const data = schema.parse(req.body);
    const dashboard = await addDashboardUser(dashboardId, data.userId, req.user.id);
    res.json(dashboard);
  } catch (err) {
    if (err.name === "ZodError") return res.status(400).json({ error: err.errors });
    next(err);
...
router.delete("/:dashboardId/users/:userId", authenticate, async (req, res, next) => {
  try {
    ...
    const dashboard = await removeDashboardUser(dashboardId, userId, req.user.id);
    res.json(dashboard);
  } catch (err) { next(err); }
});
...
router.delete("/:dashboardId", authenticate, async (req, res, next) => {
  try {
    const { dashboardId } = req.params;
    const dashboard = await deleteTelecomDashboard(dashboardId, req.user.id);
    res.json(dashboard);
  } catch (err) { next(err); }
});
