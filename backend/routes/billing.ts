...
router.get('/device-limit', authenticate, async (req: Request, res: Response, next: NextFunction) => {
...
router.post('/upgrade-mpesa', authenticate, async (req: Request, res: Response, next: NextFunction) => {
  try {
...
router.post('/extra-device-mpesa', authenticate, async (req: Request, res: Response, next: NextFunction) => {
  try {
...
router.get('/mpesa-status/:checkoutId', authenticate, async (req: Request, res: Response, next: NextFunction) => {
  try {
...
router.post('/upgrade-stripe', authenticate, async (req: Request, res: Response, next: NextFunction) => {
  try {
...
router.post('/stripe-webhook', express.raw({ type: 'application/json' }), async (req: Request, res: Response) => {
  ...
  try {
    if (event.type === 'payment_intent.succeeded') {
      ...
      const payment = await Payment.findOne({ reference: (event.data.object as Stripe.PaymentIntent).id });
