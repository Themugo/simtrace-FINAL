...
router.get("/satellite/:deviceId", authenticate, async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    const location = await getSatelliteAssistedLocation(String(deviceId));
    res.json(location);
  } catch (err) { next(err); }
});
...
router.get("/hybrid/:deviceId", authenticate, async (req: Request, res: Response, next: NextFunction) => {
  try {
    const { deviceId } = req.params;
    const location = await getHybridLocation(String(deviceId));
    res.json(location);
  } catch (err) { next(err); }
});
...
router.get("/history/:deviceId", authenticate, async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    const history = await getLocationHistory(String(deviceId), hours ? parseInt(hours as string) : 24);
