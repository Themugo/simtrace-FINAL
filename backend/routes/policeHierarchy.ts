...
// Officer/agency-only: keep regular users + telecom accounts out of police evidence/PII.
// (Citizen theft-report filing lives on the device routes, not here.)
router.use(authenticate, requireRole("law_enforcement", "admin", "super_admin"));
...
router.delete("/hierarchy/assignments/:id", authenticate, async (req: AuthRequest, res: Response, next: NextFunction) => {
  try {
    ...
    const assignment = await revokeUserAssignment(String(id), req.user!.id, reason);
    res.json(assignment);
  } catch (err) {
    if (err instanceof Error && err.name === "ZodError") return res.status(400).json({ error: (err as unknown as ZodErrorLike).errors });
    next(err);
...
router.post("/data-access/:id/approve", authenticate, async (req: AuthRequest, res: Response, next: NextFunction) => {
  try {
    ...
    const accessControl = await approveDataAccess(String(id), req.user!.id, approvalNotes ?? "");
...
router.post("/data-access/:id/revoke", authenticate, async (req: AuthRequest, res: Response, next: NextFunction) => {
  try {
    ...
    const { id } = req.params;
    const { reason } = schema.parse(req.body);
    const accessControl = await revokeDataAccess(String(id), req.user!.id, reason);
    res.json(accessControl);
  } catch (err) {
    if (err instanceof Error && err.name === "ZodError") return res.status(400).json({ error: (err as unknown as ZodErrorLike).errors });
    next(err);
...
router.post("/cooperation-alerts/check-delayed", authenticate, async (_req: AuthRequest, res: Response, next: NextFunction) => {
  try {
