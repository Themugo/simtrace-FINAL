// routes/adminRole.js - Admin role and permission API endpoints
...
router.patch("/:role/layer/:layer", authenticate, async (req, res, next) => {
  try {
    const schema = z.object({
      permissions: z.array(z.string()),
    });
...
router.patch("/:role/system-permissions", authenticate, async (req, res, next) => {
  try {
    const schema = z.object({
      systemPermissions: z.array(z.string()),
    });
...
router.post("/:role/deactivate", authenticate, async (req, res, next) => {
  try {
    const { role } = req.params;
...
router.post("/:role/activate", authenticate, async (req, res, next) => {
  try {
    const { role } = req.params;
...
// ── Permission Checking ───────────────────────────────────────────────────────────────
router.post("/:role/check-permission", authenticate, async (req, res, next) => {
  try {
    const schema = z.object({
      layer: z.enum(["end_user", "seller_reseller", "repair_shop", "telecom", "law_enforcement"]),
      permission: z.string(),
...
router.post("/:role/check-system-permission", authenticate, async (req, res, next) => {
  try {
    const schema = z.object({
      permission: z.string(),
    });
...
router.post("/initialize-default-roles", authenticate, async (req, res, next) => {
  try {
