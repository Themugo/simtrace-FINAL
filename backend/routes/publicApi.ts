...
router.get("/keys/:id", authenticate, async (req: Request, res: Response, next: NextFunction) => {
  try {
    const { id } = req.params;
    const key = await getApiKey(String(id));
...
router.patch("/keys/:id", authenticate, async (req: Request, res: Response, next: NextFunction) => {
  try {
    const { id } = req.params;
    const key = await updateApiKey(String(id), req.body);
    res.json(key);
  } catch (err) { next(err); }
});
...
router.post("/keys/:id/revoke", authenticate, async (req: Request, res: Response, next: NextFunction) => {
  try {
    const { id } = req.params;
    const key = await revokeApiKey(String(id));
    res.json(key);
  } catch (err) { next(err); }
});
...
router.delete("/keys/:id", authenticate, async (req: Request, res: Response, next: NextFunction) => {
  try {
    const { id } = req.params;
    const key = await deleteApiKey(String(id));
    res.json(key);
  } catch (err) { next(err); }
});
