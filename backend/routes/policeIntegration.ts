...
router.post("/alerts/:id/sighting", authenticate, async (req: AuthRequest, res: Response, next: NextFunction) => {
  try {
    ...
    const { id } = req.params;
    const data = schema.parse(req.body);
    const alert = await reportSighting(id as string, { ...data, reportedBy: req.user!.id });
    res.json(alert);
  } catch (err) {
    if (err instanceof Error && err.name === "ZodError") return res.status(400).json({ error: (err as any).errors });
    next(err);
...
router.post("/alerts/:id/deactivate", authenticate, async (req: AuthRequest, res: Response, next: NextFunction) => {
  try {
    const { id } = req.params;
    const alert = await deactivateAlert(id as string, req.body?.reason ?? "");
    res.json(alert);
  } catch (err) { next(err); }
});
...
router.post("/case-transfers/:id/complete", authenticate, async (req: AuthRequest, res: Response, next: NextFunction) => {
  try {
    const { id } = req.params;
    const transfer = await completeCaseTransfer(id as string);
    res.json(transfer);
  } catch (err) { next(err); }
});
